...
def update_colData(
	compound: str, colData: Dict, cpd_metadata: pd.DataFrame, database: pd.DataFrame
) -> int:
...
def main(
	...
	for compound in (cpd_pbar := tqdm.tqdm(pd.unique(signature_data['cmap_name']))):
		...
		compound_specific_signatures = signature_data[
			signature_data['cmap_name'] == compound
		]
		compound_signature_ids = list(compound_specific_signatures['sig_id'])
		cid = update_colData(
			compound=compound, colData=colData, cpd_metadata=cpd_metadata, database=adb
		)
		if cid == -1:
			continue
		...
		cpd_pbar.set_description(
			f'Working on {compound} - Data Loaded in  {np.round(e - s, 3)} seconds.'
		)
		data = data.data_df

		signatures = []
		for gene in (
			gene_pbar := tqdm.tqdm(data.index, leave=False)
		):  # iterate over genes
