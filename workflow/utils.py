...
def featurize_signatures(
        ...
        for sig_id in expression_vector.index:
            ...
            enc_batch = batch_encoder.transform(np.array(sig_batch).reshape(-1,1)).toarray()
...
def load_metadata(
    ...
    if only_named_cells:
        ...
        cell_line_metadata = cell_line_metadata.dropna(how='any',subset = ['cellosaurus_id','ccle_name'])    

    if only_cancer_cells:
        # filter for only tumor cells
        cell_line_metadata = cell_line_metadata[cell_line_metadata['cell_type']=='tumor']
    ...
    signature_data= signature_data[signature_data['pert_dose_unit'].isin(valid_dose_units)]
    ...
    common_cpds = set(signature_data['cmap_name']).intersection(set(cpd_metadata['cmap_name']))
    ...
    signature_data = signature_data.dropna(subset=['pert_time','pert_dose','bead_batch','cell_iname'])
